...
const CandidateInsert = ({
  ...
  const [work_permitfor_USA, setWork_permitfor_USA] =
    React.useState(Work_permitfor_USA);
  const [dateOfBirth, setDateOfBirth] = React.useState(DateOfBirth);
  const [permanentAddress, setPermanentAddress] =
    React.useState(PermanentAddress);
  ...
  function onChangePin_Code(event) {
    ...
    if (
      !pincodeRegex.test(enteredPincode) ||
  ...
  function onPreLocationChange(e, role) {
    const yfilter = prelocname.map((itemy) => {
      return itemy;
    });
  ...
  const getDept = (Id) => {
    fetch(
      "http://localhost:7036/Department/api/GetDeptbyIndustryId" + `/${Id}`,
      { method: "Get" }
    )
      .then((response) => response.json())
  ...
  useEffect(() => {
    const Id = 1;
    fetch(
      "http://localhost:7036/Location/api/getStatesByCountryId" + `/${Id}`,
      { method: "Get" }
    )
      .then((response) => response.json())
  ...
  const getCity = (Id) => {
    ...
    fetch(
      "http://localhost:7036/Location/api/getCitiesByStateId" + `/${cityID}`,
      { method: "Get" }
    )
      .then((response) => response.json())
  ...
  const insertRole = (value) => {
    {
      fetch("http://localhost:7163/Candidata/api/InsertKeyMapping/", {
        method: "POST",
  ...
  useEffect(() => {
    if (Data.lstPreferredLocations !== "") {
      ...
      setPreLocName(tex2);
      const temprory = temp.map((id, index) => ({
        Id: id,
        Name: tex2[index],
        IsActive: true,
      }));
  ...
  const [formFields, setFormFields] = useState([
    //    Data.candidateEducationModels
    {
      ...
      UniversityName: "",
      GraduationYear: "",
      CandidateId: CandidatId === null ? 0 : CandidatId,
      IsActive: true,
      CreatedBy: "",
      UpdatedBy: "",
      CreatedDate: "",
  ...
  const removeFields = (index) => {
    ...
    setFormFields(data);
    // console.log(data)
    let datas = [...formFields];
    datas[index].IsActive = false;
    Data.candidateEducationModels = datas;
  };
  ...
  const isDateRangeValid = (fromDate, toDate) => {
    ...
    return true; // If any date is not provided, consider it valid
  };
  ...
  const handleAddOffcial = (event, index) => {
    ...
    offdata[index]["CreatedDate"] = "2023-11-02T05:03:57.122Z";
    offdata[index]["UpdatedDate"] = "2023-11-02T05:03:57.122Z";
    const FromDate = offdata[index].FromDate; // Get FromDate
    const ToDate = offdata[index].ToDate; // Get ToDate
    if (!isDateRangeValid(FromDate, ToDate)) {
      console.log("Invalid date range. TODate should be later than FromDate.");
    } else {
      offdata[index].NoOfYearsOfExperience = calculateExperience(
        FromDate,
        ToDate
      );
      setOfficedetail(offdata);
  ...
  const calculateExperience = (FromDate, ToDate) => {
    if (FromDate && ToDate) {
      ...
      const dateDiff = to - from; // Swap the order to get a positive difference

      const oneDayMilliseconds = 24 * 60 * 60 * 1000;
      const oneMonthMilliseconds = 30.44 * oneDayMilliseconds;
      const oneYearMilliseconds = 365.25 * oneDayMilliseconds;

      const years = Math.floor(dateDiff / oneYearMilliseconds);
      const months = Math.floor(
        (dateDiff % oneYearMilliseconds) / oneMonthMilliseconds
      );
      const days = Math.floor(
        (dateDiff % oneMonthMilliseconds) / oneDayMilliseconds
      );

      const result = `${years} year ${months} month ${days} day`;
      console.log(result);
      return result;
    }
  ...
  const removeOffFields = (index) => {
    ...
    offdata.splice(index, 1);
    setOfficedetail(offdata);
    let delOff = [...officedetail];
    delOff[index].IsActive = false;
    Data.candidateEmployeeModels = delOff;
  };
  ...
  useEffect(() => {
    if (
      Data.empTabKeyskillModels !== "" &&
      Data.empTabKeyskillModels !== undefined
    ) {
  ...
  const [dupindex, setDupindex] = React.useState();
  const handleAddKeys = (e, index, value) => {
    // const handleAddKeys = (e,   index, value)
    // selectedKeySkills(value);
    setDupindex(index);
    var length = Data.empTabKeyskillModels.length - index;
    if (selectedKeySkills1 && selectedKeySkills1.length < index) {
      ...
      if (selectedKeySkills1 && selectedKeySkills1.length > 0) {
        keyset = {
          CandidateId:
            Data.empTabKeyskillModels.length > 0
              ? Data.empTabKeyskillModels[index - 1].CandidateId === CandidatId
                ? CandidatId
                : Data.empTabKeyskillModels[index - 1].CandidateId
              : CandidatId, //lstkeySkillsexperience.empTabKeyskillModel[index - 1].CandidateId,
        ...
        if (dupindex !== index) {
          if (length > Data.empTabKeyskillModels.length) {
            Data.empTabKeyskillModels.push(keyset);
          } else {
            Data.empTabKeyskillModels[index] = keyset;
          }
        } else {
          console.log(Data.empTabKeyskillModels[index]);
          // Data.empTabKeyskillModel[index]=keyset;
          Data.empTabKeyskillModels[index] = keyset;
        }
        setSelectedKeySkills(Data.empTabKeyskillModels);
        var xarray = [];
  ...
  const addNewKeySkillField = () => {
    ...
    if (
      selectedKeySkills &&
      selectedKeySkills.some((field) => !field.KeySkillId || !field.Experience)
    ) {
  ...
  const removeKeyFields = (index) => {
    ...
    let isAct = [...selectedKeySkills];
    isAct[index].IsActive = false;
    Data.candidateKeyModels = isAct;
  };
  ...
  const handleFilterOptions = (options, { inputValue }) => {
    ...
    const filteredOptions = options.filter(
      (option) => !lstgetKeyExp.includes(option)
    ); //change
  ...
  const handleNext = () => {
    if (tabValue === 0) {
      ...
      if (Data.Id == 0) {
        ...
        fetch(
          "http://localhost:7036/CandidatePersonal/api/InsertPersonalDetails",
          requestOptions
        )
          .then((response) => response.json())
          .then((resp) => {
        ...
        fetch(
          "http://localhost:7036/CandidatePersonal/api/UpdatePersonalDetails",
          requestOptions
        )
          .then((response) => response.json())
          .then((resp) => {
      ...
      const isAnyEducationEmpty = formFields.some(
        (field) =>
          !field.DegreeName ||
          !field.DegreeSpecialization ||
          !field.UniversityName ||
      ...
      if (isAnyEducationEmpty) {
        ...
        fetch(
          "http://localhost:7036/api/CandidateEducation/api/candidateEducation",
          requestOptions
        )
      ...
      if (!Data.DesignationId) {
        ...
        if (Data.Id == 0) {
          ...
          fetch("http://localhost:7036/Employee/api/insertEmployee/", {
            method: "POST",
            body: objectToFormData(Employeedetails),
          })
            .then((resp) => resp.json())
            .then((resp) => {
              setTab3("");
          ...
          fetch("http://localhost:7036/Employee/api/updateEmployee/", {
            method: "POST",
            body: objectToFormData(Employeedetails),
          })
            .then((resp) => resp.json())
      ...
      const isAnyOfficeEmpty = officedetail.some(
        (field) =>
          !field.Companyname ||
          !field.NoOfYearsOfExperience ||
          !field.Designation
      ...
      if (isAnyOfficeEmpty) {
        ...
        fetch(
          "http://localhost:7036/CandidateEmployee/api/candidateEmployee/",
          requestOptions
        )
          .then((response) => response.text())
          .then((result) => console.log(result))
  ...
  const HandleSubmit = () => {
    if (tabValue === 4) {
      ...
      fetch(
        "http://localhost:7036/EmpTabKeyskill/api/LstKeyskilssResponse/",
        requestOptions
      )
        .then((response) => response.text())
        .then((result) => console.log(result))
  ...
  const handleUpload = async (e) => {
    ...
    fetch(
      "https://localhost:7163/ResumeFile/api/UploadResumeAsync",
      requestOptions
    )
      .then((response) => response.text())
      .then((result) => console.log(result))
            ...
            <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
              ...
              >
                <Tab
                  ...
                  style={tabLabelStyle}
                ></Tab>
                <Tab label={tab2} value={1} index={1} style={tabLabelStyle} />
                <Tab label={tab3} value={2} index={2} style={tabLabelStyle} />
                <Tab label={tab4} value={3} index={3} style={tabLabelStyle} />
                <Tab label={tab5} value={4} index={4} style={tabLabelStyle} />
              </Tabs>
            </Box>
            ...
            <Card className="cart-cnr">
              ...
              <CardContent sx={{ pt: 0 }}>
                <Box sx={{ m: -1.5 }}>
                  {tabValue === 0 && (
                    <>
                      <Grid className="grid-cnr-one" container rowSpacing={2}>
                          ...
                          <table>
                                  ...
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        ...
                                        notchedOutline: isNameEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                  />
                                </Grid>
                                ...
                                <Grid container alignItems="center">
                                  ...
                                  >
                                    <FormControlLabel
                                      ...
                                      control={<Radio />}
                                      label={
                                        <span style={labelStyle}>Female</span>
                                      }
                                    />
                                    <FormControlLabel
                                      value="male"
                                      control={<Radio />}
                                      label={
                                        <span style={labelStyle}>Male</span>
                                      }
                                    />
                                    <FormControlLabel
                                      value="other"
                              ...
                              <td rowSpan="2">
                                <TextField
                                  ...
                                  InputProps={{
                                    classes: {
                                      input: "inputTextStyle",
                                      notchedOutline: isPermanentEmpty
                                        ? "redBorder"
                                        : null, // Apply the CSS class to the input element
                                    },
                                  }}
                                ...
                                <Grid container alignItems="center">
                                  ...
                                  >
                                    <FormControlLabel
                                      ...
                                      control={<Radio />}
                                      label={
                                        <span style={labelStyle}>Married</span>
                                      }
                                    />
                                    <FormControlLabel
                                      value="UnMarried"
                            ...
                            <tr>
                              <td colSpan={2}>
                                <Grid>
                                  <TextField
                                    className="sumarycustomTextField"
                                    fullWidth
                                    label="Summary"
                                    type="Summary"
                                    required
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isWorkPermitEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                  ...
                                  <td>
                                    <Grid>
                                      <TextField
                                        ...
                                        value={
                                          formFields[index].DegreeSpecialization
                                        }
                                        onChange={(event) =>
                                          handleFormChange(event, index)
                                        }
                  ...
                  {tabValue === 2 && (
                    <>
                      <Grid className="grid-cnr-two" container rowSpacing={2}>
                        <fieldset class="border rounded-3 p-3">
                          <legend class="float-none w-auto px-3">
                            Employee Details
                          </legend>
                              ...
                              <td>
                                <Grid>
                                  <Autocomplete
                                    ...
                                    renderInput={(params) => (
                                      <TextField
                                        className="customTextFieldEmploye"
                                        {...params}
                              ...
                              <td>
                                <Grid>
                                  ...
                                  <TextField
                                    ...
                                    value={Data.DateOfApplication.substring(
                                      0,
                                      10
                                    )}
                              ...
                              <td>
                                <Grid>
                                  <Autocomplete
                                    ...
                                    value={
                                      Data.RoleId !== "" ? roleName : roleName1
                                    }
                                    renderInput={(params) => (
                                      <TextField
                                        className="customTextFieldEmploye"
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isResumeHeadlineEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isCurrentSalaryEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isExpectedSalaryEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isNoticePeriodEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                ...
                                <Grid>
                                  <TextField
                                    ...
                                    InputProps={{
                                      classes: {
                                        input: "inputTextStyle",
                                        notchedOutline: isJobTitleEmpty
                                          ? "redBorder"
                                          : null, // Apply the CSS class to the input element
                                      },
                                    }}
                                  ...
                                  <td>
                                    <Grid>
                                      ...
                                      >
                                        ...
                                        <DatePicker
                                          className="customTextFieldOfficial"
                                          fullWidth
                                          format="LL"
                                          name="FromDate" // Specify the field name
                                          value={officedetail[index].FromDate}
                                          onChange={(date) =>
                                            handleAddOffcial(
                                              {
                                                target: {
                                          ...
                                          } // Pass the date and field name
                                          sx={{
                                            "& .MuiOutlinedInput-root.Mui-error .MuiOutlinedInput-notchedOutline":
                                              {
                                                height: "6rem", // Sets the border color to red when there is an error
                                                "& .MuiInput-root": {
                                                  height: "2rem",
                                                },
                                              },
                                    ...
                                    <Grid>
                                      ...
                                      >
                                        <DatePicker
                                          ...
                                          fullWidth
                                          format="LL"
                                          name="ToDate" // Specify the field name
                                          value={officedetail[index].ToDate}
                                          onChange={(date) =>
                                            handleAddOffcial(
                                              {
                                                target: {
                                          ...
                                          sx={{
                                            "& .MuiOutlinedInput-root.Mui-error .MuiOutlinedInput-notchedOutline":
                                              {
                                                height: "6rem",
                                                // Sets the border color to red when there is an error
                                                "& .MuiInput-root": {
                                                  height: "2rem",
                                                },
                                              },
                                  ...
                                  <td>
                                    <Grid>
                                      <Autocomplete
                                        ...
                                        renderInput={(params) => (
                                          <TextField
                                            className="keyExCustomTextField"
                                            {...params}
                                            name="calWexp"
